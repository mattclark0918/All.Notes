...
- (id)initWithSelectDefultGroupOnShow:(BOOL)selectDefultGroupOnShow AssetsLibrary: (ALAssetsLibrary*) assetsLib {
	_selectDefultGroupOnShow = selectDefultGroupOnShow;
	self = [self init];
    if (self) {
        self.albumPicker.assetsLibrary = assetsLib;
    }
	return self;
}
...
- (id)init
{
    ...
    if (self) {
		...
		albumPicker.selectDefultGroupOnShow = _selectDefultGroupOnShow;
		self.albumPicker = albumPicker;
...
- (void)viewDidLoad {
	[super viewDidLoad];
	[self pushViewController:self.albumPicker animated:NO];
	[self.albumPicker tableView]; // Force to load
}
