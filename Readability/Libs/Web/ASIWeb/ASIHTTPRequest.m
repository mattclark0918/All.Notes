...
- (void)startRequest
{
	...
	if(!allowCallExternalHandlersNotOnMainThread)
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			// TODO: ASIInputStream leaks on iOS7 compiled by Xcode 5.0. https://github.com/pokeb/asi-http-request/issues/378
...
/* ALWAYS CALLED ON MAIN THREAD!
 Was! // Victor Z. Added allowCallExternalHandlersNotOnMainThread 2013-09-30.
 */
- (void)reportFinished
{
...
- (void)failWithError:(NSError *)theError
{
	...
	if ([self isPACFileRequest]) {
		...
		if(!allowCallExternalHandlersNotOnMainThread) {
			[self performSelectorOnMainThread:@selector(reportFailure) withObject:nil waitUntilDone:[NSThread isMainThread]];
		} else {
			[self reportFailure];
		}
	}
...
- (void)readResponseHeaders
{
	...
	CFRelease(message);
	NSMutableDictionary *newResponseHeaders = [[[self responseHeaders] copy] autorelease];
	if(!allowCallExternalHandlersNotOnMainThread)
		[self performSelectorOnMainThread:@selector(requestReceivedResponseHeaders:) withObject:newResponseHeaders waitUntilDone:[NSThread isMainThread]];
	else
		[self requestReceivedResponseHeaders:newResponseHeaders];
}
...
- (BOOL)willAskDelegateForProxyCredentials
{
	...
	if (delegateOrBlockWillHandleAuthentication) {
		if(!allowCallExternalHandlersNotOnMainThread)
			...
			[self askDelegateForProxyCredentials];
	}
...
- (BOOL)willAskDelegateForCredentials
{
	...
	if (delegateOrBlockWillHandleAuthentication) {
		if(!allowCallExternalHandlersNotOnMainThread)
			...
			[self askDelegateForCredentials];
	}
	return delegateOrBlockWillHandleAuthentication;
}
...
- (BOOL)willAskDelegateToConfirmRedirect
{
	...
	if (needToAskDelegateAboutRedirect) {
		...
		if(!allowCallExternalHandlersNotOnMainThread)
			...
			[self requestWillRedirectToURL:newURL];
		return true;
	}
	return false;
}
...
@synthesize allowCallExternalHandlersNotOnMainThread;
