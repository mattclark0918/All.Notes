...
- (void)setBadgeText:(NSString *)badgeText
{
	...
	if(!_badgeText)
		_badgeText = @"";
	if(![_badgeText isEqual:badgeText])
	{
...
- (void) autoBadgeSizeWithString:(NSString *)badgeString
{
	...
	CGSize stringSize = [badgeString vlSizeWithFont:[UIFont boldSystemFontOfSize:12]];
	CGFloat flexSpace;
...
- (void)drawRect:(CGRect)rect
{
	...
	if ([self.badgeText length] > 0)
	{
		...
		CGSize textSize = [self.badgeText vlSizeWithFont:textFont];
		[self.badgeText vlDrawAtPoint:CGPointMake((rect.size.width/2-textSize.width/2), (rect.size.height/2-textSize.height/2)) withFont:textFont color:badgeTextColor];
	}
}
